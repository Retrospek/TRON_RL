...
import numpy as np
from tron_env import Tron2Env  # Import the environment
from dqn_agent import DQNAgent  # Import the DQNAgent class
import matplotlib.pyplot as plt

def create_training_directory():
    """Create a directory for saving training results and models."""
...
def plot_metrics(metrics, title, ylabel, save_path):
    """Plot and save the training metrics (rewards, loss, win rates)."""
...
def train_agents(episodes=1000, max_steps=500, render_interval=100, save_interval=100):
    ...
    # Create a directory for saving results and models
    save_dir = create_training_directory()
    
    # Initialize environment and agents
    env = Tron2Env()
    agent1 = DQNAgent(state_dim=env.observation_space.shape[0], action_dim=env.action_space.n, agent_id=1)
    agent2 = DQNAgent(state_dim=env.observation_space.shape[0], action_dim=env.action_space.n, agent_id=2)
    
    # Metrics for tracking the performance
    episode_rewards = []
    agent1_wins = 0
    agent2_wins = 0
    draws = 0
...
if __name__ == "__main__":
    ...
    with open(f"{results['save_dir']}/training_results.txt", 'w') as f:
        f.write("Training Results:\n")
        f.write(f"Total Episodes: {1000}\n")
        f.write(f"Agent 1 Wins: {results['agent1_wins']}\n")
        f.write(f"Agent 2 Wins: {results['agent2_wins']}\n")
        f.write(f"Draws: {results['draws']}\n")
        f.write(f"Average Reward: {np.mean(results['episode_rewards'])}\n")
